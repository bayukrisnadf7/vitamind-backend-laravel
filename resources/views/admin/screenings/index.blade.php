    ...
    <div class="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/80 shadow-xs">
        <form action="{{ route('admin.screenings.index') }}" method="GET" class="space-y-3">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                ...
                <!-- Risk Filter -->
                <div>
                    <select name="risk" class="w-full py-2.5 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm text-slate-700 focus:outline-hidden focus:border-emerald-500">
                        <option value="">Semua Tingkat Risiko</option>
                        <option value="Risiko Rendah" {{ ($risk ?? '') === 'Risiko Rendah' ? 'selected' : '' }}>Risiko Rendah</option>
                        <option value="Risiko Sedang" {{ ($risk ?? '') === 'Risiko Sedang' ? 'selected' : '' }}>Risiko Sedang</option>
                        <option value="Risiko Tinggi" {{ ($risk ?? '') === 'Risiko Tinggi' ? 'selected' : '' }}>Risiko Tinggi</option>
                    </select>
                </div>

                <!-- Action buttons -->
                <div class="flex items-center gap-2">
                    <button type="submit" class="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs sm:text-sm font-semibold rounded-xl shadow-xs transition-colors flex items-center justify-center gap-2 cursor-pointer">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if(!empty($search) || !empty($risk) || !empty($dateFrom) || !empty($dateTo))
                        <a href="{{ route('admin.screenings.index') }}" class="px-3.5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs sm:text-sm font-semibold rounded-xl transition-colors">
                            Reset
                        </a>
                    @endif
                </div>
                        ...
                        <tr class="hover:bg-slate-50/70 transition-colors">
                            <td class="py-4 px-4 sm:px-6">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <a href="{{ $sc->user ? route('admin.users.show', $sc->user->user_id) : '#' }}" class="font-semibold text-slate-900 hover:text-emerald-600 truncate block">
                            ...
                            <td class="py-4 px-4">
                                <p class="text-slate-700 font-medium">{{ $sc->created_at->translatedFormat('d M Y, H:i') }} WIB</p>
