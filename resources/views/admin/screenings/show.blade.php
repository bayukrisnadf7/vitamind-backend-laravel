        ...
        <div class="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-xs space-y-6">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-slate-100">
                ...
                <div class="flex items-center gap-4">
                    <div
                        class="w-16 h-16 rounded-2xl bg-gradient-to-tr from-emerald-500 to-teal-400 text-white flex items-center justify-center font-bold text-2xl shadow-lg shadow-emerald-500/20 shrink-0">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-1">
                            Skrining dilakukan pada <span
                                class="font-semibold text-slate-700">{{ $screening->created_at->translatedFormat('d F Y, H:i') }}
                                WIB</span>
                        </p>
                    </div>
                </div>
                ...
                <div>
                    @if ($screening->result === 'Risiko Tinggi')
                        <div
                            class="px-5 py-3.5 rounded-2xl bg-rose-50 border border-rose-200 text-rose-800 flex items-center gap-3">
                            <div
                                class="w-10 h-10 rounded-xl bg-rose-500 text-white flex items-center justify-center shrink-0 shadow-md shadow-rose-500/20">
                            ...
                            <div>
                                ...
                                <p class="text-base font-extrabold text-rose-700">Risiko Tinggi</p>
                            </div>
                        </div>
                    @elseif ($screening->result === 'Risiko Sedang')
                        <div
                            class="px-5 py-3.5 rounded-2xl bg-amber-50 border border-amber-200 text-amber-800 flex items-center gap-3">
                            <div
                                class="w-10 h-10 rounded-xl bg-amber-500 text-white flex items-center justify-center shrink-0 shadow-md shadow-amber-500/20">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                                </svg>
                            ...
                            <div>
                                <p class="text-xs font-semibold text-amber-500 uppercase tracking-wider">Status Penilaian
                                </p>
                                <p class="text-base font-extrabold text-amber-700">Risiko Sedang</p>
                            </div>
                        </div>
                    @else
                        <div
                            class="px-5 py-3.5 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 flex items-center gap-3">
                            <div
                                class="w-10 h-10 rounded-xl bg-emerald-500 text-white flex items-center justify-center shrink-0 shadow-md shadow-emerald-500/20">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div>
                                <p class="text-xs font-semibold text-emerald-500 uppercase tracking-wider">Status Penilaian
                                </p>
                    ...
                    <div class="p-5 sm:p-6 hover:bg-slate-50/60 transition-colors">
                        <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                            ...
                            <div class="flex sm:flex-col items-center sm:items-end justify-between gap-2 shrink-0">
                                <span
                                    class="inline-flex items-center px-3 py-1 rounded-xl bg-slate-100 text-slate-800 text-xs font-semibold">
                                    Jawaban: {{ $item['user_answer'] }}
                                </span>
                                {{-- <span class="text-[11px] text-slate-500">
                                    Skor: <span class="font-bold text-slate-800">{{ $item['score'] }}</span> /
                                    {{ $item['max_score'] }}
                                </span> --}}
                            </div>
                        </div>
